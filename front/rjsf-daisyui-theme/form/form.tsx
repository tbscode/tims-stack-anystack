...
const injectDataSchema = (props: InjectDataSchemaProps) => {
  ...
  Object.keys(properties).forEach((key) => {
    ...
    console.log(
      "KEY",
      key,
      historyManager.prevValues,
      historyManager.changedFields,
    );
    if (key in historyManager.prevValues)
      properties[key].previous = historyManager.prevValues[key];
    else if ("previous" in properties[key]) delete properties[key].previous;

    if (historyManager.changedFields.includes(key))
      properties[key].changed = true;
    else properties[key].changed = false;
    properties[key].changeFieldRef = key;
  });
  if ("last_updated" in properties) delete properties.last_updated;
...
const getTimeStamped = ({ ...profileData }) => {
  ...
  const updated_at = timestaped
    ? new Date(profileData.last_updated).toISOString()
    : new Date().toISOString();

  Object.keys(profileData).forEach((key) => {
    profileData[key] = updated_at;
  });
  if (updated_at) delete profileData.last_updated;
  return profileData;
};
...
export const DynamicForm = (props: DynamicFormParams) => {
  ...
  const submitData = (formData: any) => {
    ...
    if ("loading" in formData) delete formData.loading;
    if ("last_updated" in formData) delete formData.last_updated;
        ...
        res.json().then((parsed: any) => {
          if (res.ok) {
            ...
            Object.keys(parsed).forEach(function (key, index) {
              parsed[key] = { __errors: parsed[key] };
            });
            setUiSchemaParams({
              ...uiSchema,
              isFetching: false,
  ...
  const updateDisplayPrevious = ({
    ...
    const newPreDisp = display
      ? [field].concat(displayPrevious)
      : displayPrevious.filter((item) => item !== field);
    console.log(
      "UPDATE DISPLAY",
      field,
      display,
      newPreDisp,
      getPrevValues({ fields: newPreDisp }),
    );
  ...
  const revertField = ({
    ...
    setTimestamps({
      ...timestamps,
      [field]:
        "last_updated" in baseData ? baseData.last_updated : data.last_updated,
    });
  ...
  const currentSchema = injectDataSchema({
    schema,
    data,
    callbacks: { revertField, updateDisplayPrevious },
    historyManager: {
