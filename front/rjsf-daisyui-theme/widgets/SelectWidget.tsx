  ...
  return (
    <Select
      ...
      classNames={{
        ...
        input: (state) =>
          focused
            ? "bg-base-300 p-2 text-base-content text-xl input input-ghost grow"
            : "h-0",
        menu: (state) =>
          focused ? "bg-base-200 p-2 rounded-xl mt-1 shadow-2xl" : "h-0 hidden",
        control: (state) =>
          state.isFocused
            ? "flex flex-row bg-base-300 p-2 rounded-xl mt-1"
            : "flex flex-row p-2 rounded-xl mt-1 bg-base-200 hover:bg-base-300",
        multiValueLabel: (state) => "",
        indicatorsContainer: (state) =>
          focused
            ? "bg-base-100 p-2 rounded-xl h-fit absolute right-0 top-0 mr-3 mt-4 flex"
            : "hidden",
        dropdownIndicator: (state) => (menuIsOpen ? "rotate-180" : ""),
        placeholder: (state) =>
          "text-base-content text-xl flex items-center mr-4",
        multiValueRemove: (props) => (focused ? "ml-4" : "hidden"),
        valueContainer: (state) => "flex flex-row flex-wrap grow",
        multiValue: (state) =>
          "flex flex-row btn btn-sm m-1 p-1 pl-4 pr-4 h-auto normal-case text-xl",
        menuList: (state) => "flex flex-row flex-wrap",
        option: (state) =>
          "flex flex-row btn btn-sm m-1 p-1 pl-6 pr-6 h-auto normal-case text-xl",
